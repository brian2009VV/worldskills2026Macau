...
Y_range = (30, 70)  # Y-axis range of radar data
...
# Related to movement#######################################################################################
v_f = 50  # The robot's forward speed
v_h = 50  # The speed of the robot's lateral movement
f_dis = 85  # Distance when robot go through the hole
...
def get_data_to_wall(n: int, x: float, times: int) -> bool:
    WXY = []
    l = 0
    l_a, r_a = 999999, 0
...
def check_front_hole(x: float, times: int) -> bool:
    W = []
...
def go_throgh_hole(n: int, x: float) -> None:
    count = -1
    last_m = 0
    while True:
